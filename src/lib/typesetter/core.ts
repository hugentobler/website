 ...
 * - BASELINE_VAR: CSS variable name for baseline rhythm.
...
export const BASELINE_VAR = "--t8r-baseline";
const SNAP_TAGS = new Set(["H1", "H2", "H3", "H4", "H5", "H6", "P", "LI", "BLOCKQUOTE"]);

// Snap a text block's total height to the next baseline multiple.
// Needed because tuned line-heights are not always baseline multiples, and the
// correct snap depends on actual line count (multi-line headings).
const snapPaddingPx = (heightPx: number, baselinePx: number): number => {
	if (!Number.isFinite(heightPx) || !Number.isFinite(baselinePx) || baselinePx <= 0) {
		return 0;
	}
	const remainder = heightPx % baselinePx;
	const rawSnap = (baselinePx - remainder) % baselinePx;
	return rawSnap < 0.01 || Math.abs(rawSnap - baselinePx) < 0.01 ? 0 : rawSnap;
};
  ...
  if (!Number.isFinite(defaultColSpan) || defaultColSpan <= 0) {
	...
	return nodes.map((node, index) => {
		...
		const canSkipFlowBreak =
			overrideSpan !== null &&
			(node.tagName === "FIGURE" || node.tagName === "IMG");
    ...
    const colSpan = Math.min(rawColSpan, pageGridCols);
		...
		const rawHeightPx = node.getBoundingClientRect().height;
		const snapPx = SNAP_TAGS.has(node.tagName)
			? snapPaddingPx(rawHeightPx, rowGapPx)
			: 0;
		if (snapPx > 0) {
			node.style.setProperty("--t8r-snap", `${snapPx}px`);
		} else {
			node.style.removeProperty("--t8r-snap");
		}
		const heightPx = rawHeightPx + snapPx;
      ...
      for (let pageIndex = 0; pageIndex < pages.length; pageIndex += 1) {
        ...
        const slot = findFirstFlowSlot(
          ...
          flowCursor && pageIndex === flowCursor.pageIndex
            ? { col: flowCursor.col, row: flowCursor.row }
            : null,
        );
        if (!slot) continue;
...
const groupFlowNodes = (
  ...
  return { consumedIndexes, groupedNode };
};
