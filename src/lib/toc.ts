/**
 * Shared TOC (table of contents) computation.
 *
 * Extracts h2/h3 headings from a Markdoc module and computes section
 * sizes from the raw markdown source. Used by both the catchall article
 * route and the nook route.
 */
...
/**
 * Build a TOC from a Markdoc module's headings and its raw markdown source.
 * `rawModules` is a Vite glob map of `?raw` markdown imports — the caller
 * passes whichever glob covers the content directory in question.
 */
export async function loadToc(
	slug: string,
	headings: MarkdocModule["headings"],
