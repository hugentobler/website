...
const nodes: Config["nodes"] = {
			...
			if (raw.length === 1) {
				...
				if (only?.type === "inline" && Array.isArray(only.children)) {
					const inlineChildren = only.children.filter(
						(child) =>
							!(child?.type === "text" && String(child?.attributes?.content ?? "").trim() === ""),
					);
	...
	table: {
		...
		transform(node, config) {
			...
			// tables remain readable on narrow viewports instead of being
			// squeezed to fit the text column.
			return new Markdoc.Tag("div", { class: "table-scroll" }, [
