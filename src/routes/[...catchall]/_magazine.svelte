...
<article
  bind:this={article}
  class="relative scroll-timeline-y lg:scroll-timeline-x"
>
  <div id="top" class="absolute top-0"></div>
  <ScrollIndicator
    class="scroll-indicator top-0 animate-fade-in"
    direction="up"
    href="#top"
  />
        ...
        {#each filteredHeadings as heading (heading.id)}
          <li class=" flex-1 !text-base">
            <span class="line-clamp-2 text-pretty">
              <DecoratedLink
                class="!text-(--foreground)/70 hover:!text-(--foreground)/100"
                href={`#${heading.id}`}
              >
                {heading.title}
              </DecoratedLink>
...
<style lang="postcss">
  ...
  article {
    ...
    @media (width >= theme(--breakpoint-lg)) {
      ...
      columns: calc(
        min(65ch, (100cqw - var(--x-spacer) * 2) / var(--visible-columns))
      ) !important;
    }
