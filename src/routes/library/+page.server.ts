...
import { HARDCOVER_API_BEARER_TOKEN, LITERAL_EMAIL, LITERAL_PASSWORD } from '$env/static/private';
...
async function getHardcoverBooks(): Promise<{ books: HardcoverActivity[]; error?: string }> {
  try {
    ...
    if (!result.data?.me?.[0]?.activities) {
      return { books: [], error: 'No book activities found in Hardcover response' };
    }
...
async function getLiteralBookData(isbns: string[]): Promise<ValidLiteralBook> {
  ...
  throw new Error(`Could not find valid book data for any ISBN: ${isbns.join(', ')}`);
}
...
async function getBooks(): Promise<LibraryItem[]> {
  ...
  for (const activity of hardcoverBooks) {
    const isbns = activity.book.editions.map((edition) => edition.isbn_13).filter(Boolean);
    if (isbns.length === 0) {
    ...
    try {
      ...
      const libraryItem: LibraryItem = {
        ...
        published_by: literalBook.authors.map((author) => author.name).join(', '),
      ...
      // Continue processing other books - just omit this one from the library
      continue;
    }
  }
